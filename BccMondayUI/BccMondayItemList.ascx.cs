...
namespace RockWeb.Plugins.com_baysideonline.BccMondayUI
{
    ...
    public partial class BccMondayItemList : RockBlock
    {
            ...
            using (var context = new RockContext())
            {
                ...
                if (requestorEmail.IsNotNullOrWhiteSpace() && chosenBoard.HasValue)
                {
                    ...
                    if (result != null && result.Count > 0)
                    {
                        ...
                        {
                            items = items.Where(i => string.Equals(
                                i.GetRequestorEmail(Board.EmailMatchColumnId)
                                , requestorEmail,
        ...
        public List<IItem> SortItems(SortProperty sortProperty, List<IItem> items, int statusIndex)
        {
